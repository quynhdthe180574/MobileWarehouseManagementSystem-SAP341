...
import 'transfer_posting_screen.dart';     // ← thêm
import 'history_screen.dart';               // ← thêm
...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final menus = [
      ...
      // ───────────────────────────────────────────────
      // Các mục mới được thêm vào phía dưới
      // ───────────────────────────────────────────────
      _MenuItem(
        title: "Chuyển kho (Transfer Posting)",
        subtitle: "POST /GoodsMovementSet (311)",
        icon: Icons.swap_horiz,
        color: Colors.teal,
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => const TransferPostingScreen()),
          );
        },
      ),
      _MenuItem(
        title: "Lịch sử giao dịch",
        subtitle: "GET /GoodsMovementSet or HistorySet",
        icon: Icons.history,
        color: Colors.indigo,
        onTap: () {
          Navigator.push(
